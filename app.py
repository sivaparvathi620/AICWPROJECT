...
def analyze_report_with_groq(category, status, confidence):
    if client is None:
        return (
            "[ENGLISH]\n"
            f"- What we see: The {category} scan was classified as {status}.\n"
            "- Why it matters: This is an automated screening result, not a diagnosis.\n"
            "- What to do next: Please discuss this result with a qualified healthcare professional.\n\n"
            "[TELUGU]\n"
            "- తదుపరి ఏమి చేయాలి: ఇది ఆటోమేటెడ్ స్క్రీనింగ్ ఫలితం మాత్రమే. వైద్య నిపుణుడిని సంప్రదించండి."
        )
...
model_paths = {
    'brain': 'models/brain_tumor_model.h5',
    'pneumonia': 'models/chest_model.h5',
    'retina': 'models/retina_model.h5',
    'skin': 'models/skin_cancer_model.h5',
}

for category, model_path in model_paths.items():
    full_model_path = os.path.join(base_dir, model_path)
    try:
        MODELS[category] = tf.keras.models.load_model(full_model_path)
        print(f"✅ {category.title()} model loaded")
    except Exception as e:
        print(f"⚠️ {category.title()} model unavailable: {e}")
